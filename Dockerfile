...
RUN cp /usr/src/webservice/runit.d/nginx.run.sh /etc/service/nginx/run &&\
    ...
    cp /usr/src/webservice/piereling/piereling.wsgi /etc/piereling.wsgi &&\
    chmod a+x /etc/piereling.wsgi &&\
    cp -f /usr/src/webservice/piereling.nginx.conf /etc/nginx/http.d/default.conf
